...
export type CustomAlertProps = {
    type: CustomAlertType.SUCCES | CustomAlertType.ERROR;
    title: string;
    message: string
}
...
export function CustomAlert({ type, title, message }: CustomAlertProps) {
    ...
    const alert = type === CustomAlertType.SUCCES
...
function SuccessAlert({ title, message }: SuccesAlertProps) {
    return (
        <Alert className="flex-row border-emerald-500">
            <CheckIcon className="h-8 w-8 mx-4 stroke-emerald-500"/>
